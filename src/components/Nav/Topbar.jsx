...
import { getNotification ,read, rejectNotify} from '../../service/Notification'
  ...
  <div className="container-fluid">
    ...
    <div className="collapse navbar-collapse " id="navbarTogglerDemo02">
      ...
      <li className="nav-item mx-2">
        ...
        <li className="nav-item mx-2">
          <Link className="nav-link" to="/commonlayout/connect">Connect</Link>
        </li>
        <li className="nav-item mx-2">
          <Link className="nav-link " to="/commonlayout/adduser">Add User</Link>
        </li>
        <li className="nav-item mx-2" onClick={()=>{handleNotification()}}>
          <div className="nav-link d-flex">
          <span  to="#">Notification</span>
           {count.length > 0 ?<div className='counter'>{count.length}</div> : null }
    ...
    <ul className='list'>
      { 
        notification.map((item,index)=>(
          <li className='connectionlist py-1' style={{cursor:"pointer"}} onClick={()=>{handleRead(item)}}>{item.type==="1" ?
           <div className='d-flex justify-content-around my-2' >
           {item.read==="unread" ? <b>{item.message}</b> :
           <p>{item.message}</p>} 
          <button className='btn btn-success mx-2' onClick={()=>{handleConnect(item)}}><i class="fa fa-check" aria-hidden="true" ></i></button>
