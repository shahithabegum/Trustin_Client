...
const Connect = () => {
    ...
    const fetchdata=(value)=>{
      const result= company.filter((user)=>{
        return value && user && user.useremail && user.useremail.toLowerCase().includes(value) 
      })
      console.log(result)
      setDataList(result)
    }
    ...
    return (
        <div>
          ...
          <h2 className="Companyheader text-center my-5"> Connect with another Company </h2>
                    ...
                    <Col>
                     <div className='list_wrapper mx-auto'>
                      {datalist.map((item,index)=>(
                        <div key={index} className='connnect_list d-flex justify-content-between mt-3' onClick={()=>{setEmail(item.useremail)}}>
                            <div className='mx-2'>
                     ...
                     <Link to="/commonlayout/landingpage" className='my-3'><button className='btn btn-warning w-100 my-3 mx-1'>Back</button></Link>
                    </Col>
                  </Row>
